 ...
 * Detects jest.mock()/vi.mock() targeting non-boundary modules
 */
import { basename, relative } from "node:path";
import { globSync } from "glob";
import { TddGuardConfig, Violation } from "../types.js";
import { parseFile } from "../parser.js";

export function ruleNoInternalMocks(
  _srcFiles: string[],
  testFiles: string[],
  ...
  for (const file of testFiles) {
    ...
    for (const mock of parsed.mockCalls) {
      ...
      const isBoundary = boundaryModules.some(
        (b) =>
          target === b ||
          target.includes(`/${b}`) ||
          target.includes(b) ||
