...
export function mutate(options: MutateOptions): MutateResult {
...
function detectLanguage(srcDir: string): string {
  if (!existsSync(srcDir)) {
    ...
    if (existsSync("package.json")) return "typescript";
    return "unknown";
  }
...
function runStryker(
  ...
  const hasStryker =
    existsSync(resolve("node_modules/@stryker-mutator/core")) ||
  ...
  if (!hasStryker) {
    return {
      ...
      exit_code: 1,
      details:
        "Stryker not installed. Run: npm install --save-dev @stryker-mutator/core @stryker-mutator/jest-runner",
    };
  }
  ...
  try {
    ...
    for (const line of output.split("\n")) {
      ...
      try {
        ...
        if (entry.outcome === "killed") killed++;
        else if (entry.outcome === "survived") survived++;
        else if (entry.outcome === "timeout") timeout++;
        else if (entry.outcome === "unviable") unviable++;
      } catch {
        continue;
      }
    }
