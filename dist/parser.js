...
const traverse = ("default" in traverseModule.default
...
export function parseTestFile(filePath) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const testMatch = line.match(/^\s*(x?(?:test|it|describe|specify))(?:\.(?:skip|todo|only))?\s*[\s(]/);
        if (testMatch) {
            const type = testMatch[1].replace(/^x/, "");
            ...
            // Extract test name
            const nameMatch = line.match(/(?:test|it|describe|specify)(?:\.(?:skip|todo|only))?\s*[\s(]\s*["'`]([^"'`]+)["'`]/);
        ...
        if (inTest && currentTest) {
            ...
            if (/\b(expect|assert|assertThat|should|assert_eq!|self\.assert)\s*[.(!]/.test(line)) {
 ...
 */
export function parseRustTestFile(filePath) {
    const code = readFileSync(filePath, "utf-8");
    const testBlocks = [];
    const imports = [];
    const mockCalls = [];
 ...
 */
export function parsePythonTestFile(filePath) {
    const code = readFileSync(filePath, "utf-8");
    const testBlocks = [];
    const imports = [];
    const mockCalls = [];
...
export function parseFile(filePath) {
    ...
    if (ext === "rs")
        return parseRustTestFile(filePath);
    if (ext === "py")
