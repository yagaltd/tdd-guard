...
export function ruleTestPerExport(srcFiles, testFiles, config) {
    const violations = [];
    ...
    for (const exp of exports) {
        ...
        const isReferenced = allTestText.includes(name) ||
...
function collectExports(srcFiles, publicExports) {
    const exports = [];
    for (const file of srcFiles) {
        ...
        if (file.endsWith(".rs")) {
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i];
                const match = line.match(/pub\s+(?:async\s+)?fn\s+(\w+)|pub\s+struct\s+(\w+)|pub\s+enum\s+(\w+)/);
