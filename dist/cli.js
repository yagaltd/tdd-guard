...
function printLintText(result) {
    ...
    console.log(`Result: ${errors > 0 ? "FAIL" : "PASS"} (${errors} error${errors !== 1 ? "s" : ""}, ${warns} warning${warns !== 1 ? "s" : ""})`);
}
function printVerifyText(result) {
    ...
    console.log("=".repeat(40));
    console.log(`Result: ${result.exit_code === 0 ? "PASS" : "FAIL"}`);
}
function printMutateText(result) {
    ...
    console.log("=".repeat(40));
    console.log(`Result: ${result.mutation_score >= result.threshold ? "PASS" : "FAIL"} — score ${result.mutation_score}% < threshold ${result.threshold}%`);
}
program.parse();
